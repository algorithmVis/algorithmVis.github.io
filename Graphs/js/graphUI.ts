...
$("#graphUI").click(function (e: any) {
    ...
    /** If div was clicked */
        ...
        clickedId = -1;
        deselectAllNodes();
        let parent = $(this).offset();
        let x: number = Math.round(e.pageX - parent.left);
        let y: number = Math.round(e.pageY - parent.top);
        graphUIClicked(x, y);
    }
...
function addEdge(id: number, n1: number, n2: number) {
    edgeIdList[n1][n2] = id;
    edgeIdList[n2][n1] = id;
    let r: number = getNodeWidth(n1) / 2;
    let p1 = getNodePosition(n1), p2 = getNodePosition(n2);
    let line = parseLine(id, p1.left + r, p1.top + r, p2.left + r, p2.top + r);
...
function addWeightedEdge(id: number, n1: number, n2: number, weight: number) {
    addEdge(id, n1, n2);
    let edge = $("#edge" + id);
    let x1: number = parseInt(edge.attr("x1")), y1: number = parseInt(edge.attr("y1"));
    let x2: number = parseInt(edge.attr("x2")), y2: number = parseInt(edge.attr("y2"));
    let x: number = x1 + (x2 - x1) / 2, y: number = y1 + (y2 - y1) / 2;
    let normal = getNormal(x1, y1, x2, y2);
    let unit = getUnit(normal.x, normal.y);
    let unitScale = 25;
...
function addQueueElement(id: number) {
    let queueSize = $("#queueUI").find("div").length;
    let top = queueSize * 90 + 5;

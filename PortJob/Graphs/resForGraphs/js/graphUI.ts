...
$("#graphUI").click(function(e) {
    ...
    function nodeClicked(id:number) {
        if (clickedId == -1) {
            ...
            setTimeout(function() {deselectTwoNodes(n1, n2); }, 800);
            clickedId = -1;
        }
    }
});
...
function popQueueElement(id: number) {
    $("#queueNode" + id).animate({left: "200px"}, 700, function() { $(this).remove(); });
...
function resetGraphUI() {
    $("#graphUI div.nodeUI").each(function() { $(this).remove(); });
    $("#graphUI p.edgeWeight").each(function() { $(this).remove(); });
    $("#edgeSvg line").each(function() { $(this).remove(); });
    $("#queueUI div").each(function() { $(this).remove(); });
    edgeIdList = [[]];
}
